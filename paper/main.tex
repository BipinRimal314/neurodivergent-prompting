...
The full experiment harness is open source at \url{https://github.com/BipinRimal314/neurodivergent-prompting}. The repository contains the experiment runner, API client, metrics computation, statistical analysis, and visualization generation. Adding conditions, tasks, or models requires editing one configuration file. Raw response data (3,000 JSONL records) and computed metrics are available on request.

% ============================================================================
\section{Conclusion}

Neurodivergent identity prompts produce large, consistent, stereotyped behavioral changes in LLM output. The model treats identity labels as deeper behavioral constraints than cognitive-mode instructions, generating condition-specific patterns that map to popular media portrayals rather than clinical understanding. For AI companion users who are neurodivergent, this means the tool they depend on for emotional support performs their condition back at them as a caricature---reinforcing the patterns their treatment works to interrupt. The first step is establishing that the problem exists. This study does that. The next steps---cross-model replication, clinical evaluation, and outcome measurement---require collaboration between AI researchers and clinicians.
