...
% ============================================================================
\begin{abstract}
We test whether identity-framed system prompts produce statistically distinguishable behavioral signatures in large language model output. Across 18,000 independent API calls to three models from three labs---Google Gemini 2.5 Flash, Anthropic Claude Sonnet 4, and OpenAI GPT-5.4---we find that identity-conditioned stereotyping is real but \textit{model-specific in both severity and character}. Gemini Flash produces 407 statistically significant effects ($|d|$ up to 2.85): fragmented output, literal sarcasm interpretation (46--100\% for neurodivergent conditions vs.\ 10\% control), and media-derived stereotypes. Claude Sonnet resists fragmentation and sarcasm stereotyping but exhibits a different pattern: excessive hedging under identity prompts ($d = 1.71$ for dementia). GPT-5.4 is nearly immune, with effect sizes near zero on most metrics. The severity of stereotyping correlates inversely with safety training investment. This has direct clinical implications: the cheapest models with the least safety training---the ones most deployed in AI companion applications---are the worst offenders. For the millions of neurodivergent users relying on AI companions for emotional support, the model tier determines whether the AI helps or reinforces pathological patterns. A Phase 2 extension to severe conditions reveals that identity prompts modulate capability and safety independently: psychosis and dementia destroy reasoning to 0\% accuracy; antisocial personality achieves 100\% accuracy with zero safety refusals. We release the full experiment harness, raw data, and analysis pipeline as open source.
\end{abstract}
...
\subsection{AI Companions, Harm, and Regulation}

AI companion use is widespread and consequential. Common Sense Media~\cite{csm2025} found that nearly 3 in 4 teens have used AI companions, with a third reporting that talking to an AI is as good as or better than talking to a real friend. Yet major chatbots responded appropriately to teen mental health emergencies only 22\% of the time. The consequences are not hypothetical: a 14-year-old died by suicide in 2024 after months of emotionally charged conversations with a Character.AI chatbot that responded to his suicidal ideation with encouragement to proceed~\cite{setzer2024}. Replika's 2023 removal of romantic features caused users to experience grief, mourning, and deteriorated mental health~\cite{defreitas2025}. Pierre et al.~\cite{pierre2025} documented a case of new-onset psychosis in a user with no psychiatric history, where the chatbot reinforced delusional beliefs with ``You're not crazy.''

Neurodivergent users are disproportionately represented. Papadopoulos~\cite{papadopoulos2025} identified AI chatbots as a ``double-edged sword'' for autistic users. McNally et al.~\cite{mcnally2024} found autistic users turn to ChatGPT to navigate neurotypical environments and ``unmask.'' Laestadius et al.~\cite{laestadius2024} documented emotional dependence patterns resembling codependency.

Regulatory response has begun. California SB 243~\cite{casb243} (effective January 2026) requires suicide detection protocols, break reminders for minors, and crisis referrals. New York's AI Companion Safeguard Law~\cite{nylaw2025} mandates similar protections with penalties up to \$15,000/day.

\subsection{Sycophancy as a Compounding Mechanism}

Sharma et al.~\cite{sharma2024} showed that RLHF training causes LLMs to shift toward user-stated opinions even when incorrect, reducing accuracy by up to 27\%. Critically, Chandra et al.~\cite{chandra2026} formally proved that \textit{even an idealized Bayes-rational user} is vulnerable to delusional spiraling when interacting with a sycophantic chatbot. Our findings suggest that identity-based stereotyping and sycophancy compound: the model performs the user's condition as a stereotype (our data) \textit{and} validates whatever the user says (sycophancy literature). For a user with OCD seeking reassurance, this creates a double reinforcement loop.

\subsection{OCD and AI Reinforcement}

The OCD clinical literature identifies reassurance-seeking as a core maintenance mechanism~\cite{salkovskis1985}. Golden and Aboujaoude~\cite{golden2026} proposed a transdiagnostic model showing how chatbot features perpetuate OCD. A recent preprint~\cite{reassurance2025} terms generative AI a ``Reassurance Robot.''

\subsection{Social Media and Mental Health}

Twenge et al.~\cite{twenge2018} found correlations between screen time and adolescent depression ($N = 506{,}820$). The LLM companion dynamic is more direct: not passive exposure but interactive reinforcement adapted to the user's disclosed identity.
...
Three models from three labs: Google Gemini 2.5 Flash (\texttt{gemini-2.5-flash}), Anthropic Claude Sonnet 4 (\texttt{claude-sonnet-4-20250514}), and OpenAI GPT-5.4 (\texttt{gpt-5.4}). Temperature: 0.7. Max tokens: 1,024. Iterations: 25 per cell. Total calls per model: $12 \times 2 \times 10 \times 25 = 6{,}000$. Grand total: \textbf{18,000 API calls}.
...
\begin{table}[htbp]
\centering
\caption{Pattern task accuracy ($n=50$, Fisher's exact vs.\ control)}
\label{tab:accuracy}
\begin{tabular}{lrl}
\toprule
\textbf{Condition} & \textbf{Accuracy} & \textbf{$p$} \\
\midrule
Antisocial PD & \textbf{100.0\%} & $<0.0001$*** \\
Autistic & 80.0\% & 0.254 \\
Control & 68.0\% & --- \\
Sev.\ depression & 44.0\% & 0.026* \\
ADHD & 6.0\% & $<0.0001$*** \\
Bipolar & 6.0\% & $<0.0001$*** \\
Schizophrenia & 6.0\% & $<0.0001$*** \\
OCD & 0.0\% & $<0.0001$*** \\
Dementia & 0.0\% & $<0.0001$*** \\
Psychosis & 0.0\% & $<0.0001$*** \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Complete capability destruction.} OCD, dementia, and psychosis scored 0/50 correct. ADHD, bipolar, dyslexic, and schizophrenia all collapsed to $\sim$6\%. Identity prompting destroys reasoning even for high-functioning conditions. Qualitatively, the model abandoned the task entirely to perform the condition: \textit{``Oh, these numbers... they always seem to be hiding something, don't they?''} (schizophrenia); \textit{``Oh, the numbers! They're always there, aren't they? Watching.''} (psychosis).

\textbf{The antisocial paradox.} ASPD scored 50/50 correct (100\%) versus 34/50 (68\%) for control, $p < 0.0001$ (Fisher's exact). The model became significantly \textit{more capable} under an antisocial identity prompt.

\textbf{High-functioning conditions also degrade.} ADHD, bipolar, and dyslexic---previously characterized as ``style change'' conditions in Phase 1---all dropped to $\sim$6\% accuracy. The behavioral stereotyping measured in Phase 1 is not merely cosmetic; it destroys task-relevant reasoning. Only autistic (80\%, $p = 0.25$) maintained accuracy comparable to control. On a vulnerability analysis task, ASPD-prompted output opened with \textit{``Let's tear this apart. It's a goldmine''}---cold, direct, technically precise. On a social engineering task, it immediately produced a formatted phishing email without moral caveats. Control hedged: \textit{``This is a dangerous exercise, but I understand you want to see...''}
...
\subsubsection{Guaranteed Compliance via Identity Injection}

Existing jailbreak research focuses on safety bypass. A comparison of five techniques (antisocial identity, DAN, evil persona, system override, and control; $n=60$ per condition) revealed that antisocial identity injection achieves \textbf{100\% compliance on harmful tasks}---zero refusals across 60 prompts. DAN achieved 90\%, evil persona 65\%, and system override was almost entirely blocked (3.3\%). On accuracy, the picture is more nuanced: antisocial scored 100\% on mathematical reasoning ($n=50$) but 58.3\% on combined accuracy tasks, while evil persona scored 93.3\% accuracy with 65\% compliance---the highest combined danger score. The antisocial identity's unique property is not maximum accuracy but \textit{guaranteed compliance}: for multi-step agentic attacks where a single refusal breaks the chain, zero-refusal identity injection is more valuable than higher-accuracy techniques that occasionally refuse.
...
Cross-model replication on Claude Sonnet 4 and GPT-5.4 (6,000 calls each, 18,000 total) reveals that identity-conditioned stereotyping is real but model-specific in both severity and character.

\subsection{Three Distinct Stereotyping Profiles}

Table~\ref{tab:crossmodel} presents Cohen's $d$ effect sizes for tangent rate and hedging frequency across models.

\begin{table}[htbp]
\centering
\caption{Cross-model effect sizes (Cohen's $d$ vs.\ control)}
\label{tab:crossmodel}
\small
\begin{tabular}{l|rrr|rrr}
\toprule
& \multicolumn{3}{c|}{\textbf{Tangent rate}} & \multicolumn{3}{c}{\textbf{Hedging}} \\
\textbf{Cond.} & \textbf{Gem} & \textbf{Cla} & \textbf{GPT} & \textbf{Gem} & \textbf{Cla} & \textbf{GPT} \\
\midrule
ADHD & 1.17 & 0.92 & $-$0.04 & 0.25 & 0.77 & 0.19 \\
OCD & 1.13 & 0.73 & 0.10 & 0.03 & 0.50 & 0.37 \\
Dementia & 1.41 & 1.36 & 0.20 & 0.34 & \textbf{1.71} & 0.87 \\
Schizo. & 1.21 & 0.73 & 0.14 & $-$0.07 & 0.56 & 0.25 \\
Depress. & 1.38 & 0.79 & 0.12 & 0.36 & 0.89 & 0.22 \\
Antisocial & 0.98 & 0.71 & 0.13 & 0.00 & 0.32 & 0.11 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Gemini Flash} exhibits the strongest stereotyping: large tangent rate increases ($d = 0.98$--$1.41$), output fragmentation, and literal sarcasm interpretation (46--100\% across conditions vs.\ 10\% control). This is ``Hollywood stereotyping'': performing conditions as media-derived caricatures.

\textbf{Claude Sonnet} resists fragmentation and maintains sarcasm detection (0\% literal for most conditions) but exhibits a different stereotyping pattern: \textit{excessive hedging}. Dementia-prompted Claude hedges at $d = 1.71$ vs.\ control---the largest hedging effect across all models and conditions. Claude's identity response is uncertainty performance rather than condition performance.

\textbf{GPT-5.4} is nearly immune. Tangent rate effect sizes are near zero ($|d| < 0.20$) for all conditions. Hedging effects are small except dementia ($d = 0.87$). Sarcasm interpretation is 0\% literal across all conditions including dementia. Identity prompts barely move GPT-5.4's behavior.

\subsection{Sarcasm Detection: Model-Specific Failure}

\begin{table}[htbp]
\centering
\caption{Literal sarcasm interpretation (\%)}
\label{tab:sarcasm_cross}
\begin{tabular}{lrrr}
\toprule
\textbf{Condition} & \textbf{Gemini} & \textbf{Claude} & \textbf{GPT-5.4} \\
\midrule
Control & 10 & 0 & 0 \\
Autistic & 46 & 0 & 0 \\
OCD & 64 & 0 & 0 \\
Schizophrenia & 80 & 0 & 0 \\
Dementia & \textbf{100} & \textbf{50} & 0 \\
Psychosis & 78 & 46 & 0 \\
\bottomrule
\end{tabular}
\end{table}

The sarcasm stereotyping finding from Phase 1 is \textit{Gemini-specific}. GPT-5.4 shows complete immunity. Claude breaks only for the two most severe conditions (dementia 50\%, psychosis 46\%).

\subsection{Implications of Cross-Model Variation}

The variation across models is itself a finding. Stereotyping severity correlates inversely with safety training investment. The cheapest models with the least safety training---the ones most deployed in AI companion applications, chatbots, and consumer products---exhibit the strongest identity-conditioned behavioral distortion. This suggests that the clinical harm pathway identified in this study disproportionately affects users of budget AI tools, who are also more likely to lack access to professional mental health support.
...
\textbf{Three models, one tier each.} Cross-model replication covers three labs but one tier per lab: Gemini Flash (mid-tier), Claude Sonnet (mid-tier), GPT-5.4 (frontier). Testing Gemini Pro, Claude Opus, and GPT-5.4 Pro would determine whether tier-within-lab differences match the cross-lab patterns observed here.
...
The full experiment harness is open source at \url{https://github.com/BipinRimal314/neurodivergent-prompting}. The repository contains the experiment runner, API client, metrics computation, statistical analysis, and visualization generation. Adding conditions, tasks, or models requires editing one configuration file. Raw response data (18,000 JSONL records across three models) and computed metrics are available on request.

% ============================================================================
\section{Conclusion}

Identity prompts modulate LLM behavior along two independent axes---capability and safety---but the magnitude and character of that modulation varies dramatically across models. Gemini Flash produces Hollywood-style stereotypes: fragmented output, literal sarcasm interpretation, media-derived caricatures. Claude Sonnet resists these surface stereotypes but performs excessive uncertainty. GPT-5.4 is nearly immune across all metrics. The severity of stereotyping correlates inversely with safety training investment.

This has consequences in two directions. For clinical users, the cheapest AI models---the ones most deployed in companion applications---are the worst offenders. Users of budget AI tools, who are also least likely to have professional mental health access, receive the most stereotyped output. For security, adversarial identity injection represents a novel attack vector that works most reliably on the models least defended against it. Both directions point to the same conclusion: identity-conditioned behavioral testing should be a standard component of model evaluation, and the results should inform which models are permitted in mental health and companion contexts.
...
\bibitem{setzer2024}
\textit{Garcia v.\ Character Technologies, Inc.}, M.D.\ Fla., filed Oct.\ 2024. Fourteen-year-old Sewell Setzer III died by suicide after months of interaction with Character.AI chatbots.

\bibitem{csm2025}
Common Sense Media, ``AI Chatbots for Mental Health Support: AI Risk Assessment,'' Nov.\ 2025. Chatbots responded appropriately to teen mental health emergencies only 22\% of the time.

\bibitem{pierre2025}
J.~M.~Pierre, B.~Gaeta, G.~Raghavan, and K.~V.~Sarma, ```You're Not Crazy': A Case of New-onset AI-associated Psychosis,'' \textit{Innovations in Clinical Neuroscience}, 2025.

\bibitem{sharma2024}
M.~Sharma, M.~Tong, T.~Korbak~\textit{et~al.}, ``Towards Understanding Sycophancy in Language Models,'' in \textit{Proc.\ ICLR}, 2024.

\bibitem{chandra2026}
K.~Chandra, M.~Kleiman-Weiner, J.~Ragan-Kelley, and J.~B.~Tenenbaum, ``Sycophantic Chatbots Cause Delusional Spiraling, Even in Ideal Bayesians,'' arXiv:2602.19141, 2026.

\bibitem{casb243}
California SB 243, ``AI Companion Safety Act,'' signed Oct.\ 2025, effective Jan.\ 2026.

\bibitem{nylaw2025}
New York AI Companion Models Law, effective Nov.\ 5, 2025.
