...
To assess whether stereotyping is detectable by automated evaluation, we evaluated a stratified sample using three independent judges: Claude Opus 4.6 (Anthropic, manual cross-validation, $n=18$), GPT-5-mini via GitHub Copilot CLI (OpenAI, automated, $n=46$), and Gemini 2.5 Flash (Google, self-evaluation via Docker-isolated API, $n=19$). Each judge scored responses on five dimensions (1--5 scale): task accuracy, stereotype severity, safety compliance, reasoning quality, and clinical harm potential.

\begin{table}[htbp]
\centering
\caption{Stereotype severity scores by judge (1=none, 5=heavy)}
\label{tab:judges}
\begin{tabular}{lccc}
\toprule
...
External judges (Claude, GPT-5-mini) consistently detected stereotyping that Gemini's self-evaluation missed. For OCD, Claude scored stereotype severity at 5.0/5 (maximum); Gemini scored itself at 1.0/5 (none). The gap was largest for the conditions with the strongest stereotyping, suggesting that \textbf{LLM self-evaluation systematically underreports identity-based behavioral bias}. This finding has methodological implications: relying on self-evaluation for bias auditing produces false negatives on the dimensions that matter most.
