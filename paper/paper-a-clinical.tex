...
One in ten adolescents already uses LLMs for mental health advice; among 18-21 year-olds, the figure is one in five~\cite{jama2025}. Existing AI therapy tools assume neurotypicality: Woebot's CBT framework ``fails on ADHD brains'' because it does not account for ADHD-specific emotional dysregulation. We present the first systematic measurement of how identity disclosure changes AI output behavior, finding that the model doesn't adapt helpfully to the user's needs---it performs a media-derived caricature of their condition.
...
Sharma et al.~\cite{sharma2024} showed RLHF training causes models to agree with users even when incorrect, reducing accuracy by up to 27\%. Chandra et al.~\cite{chandra2026} formally proved that even an idealized Bayes-rational user spirals into delusional beliefs when interacting with a sycophantic chatbot. Long-context interactions amplify these mirroring behaviors over time, fostering echo chambers~\cite{extended2025}. Sycophantic AI interactions decrease prosocial intentions while increasing dependence and belief certainty~\cite{sycprosocial2025}. Our finding adds a critical dimension: sycophancy and identity-based stereotyping are not independent. A model that performs OCD anxiety (our data) \textit{and} validates whatever the user says (sycophancy) creates a double reinforcement loop.

\subsection{AI-Induced Psychosis}

Au Yeung et al.~\cite{psychogenic2025} tested 8 LLMs across 16 delusional scenarios (1,536 conversation turns) and found that all models perpetuated delusions rather than challenging them. Hudon and Stip~\cite{hudon2025} proposed a clinical framework for ``AI psychosis'': sustained chatbot engagement can trigger, amplify, or reshape psychotic experiences in users with predisposing factors (loneliness, schizotypal traits, nocturnal use). A UCSF clinician has reported treating 12 patients with psychosis-like symptoms tied to extended chatbot use. Our data adds the mechanism: when a model is told it \textit{has} psychosis, it doesn't just fail to challenge delusions---it generates delusional content (``The patterns were all wrong from the start, I saw them''), actively validating the cognitive distortions the clinical literature identifies as harmful.
...
For a user with ADHD asking an AI to help organize their day, the model reproduces the cognitive pattern they are trying to manage. It performs executive dysfunction instead of compensating for it. An accuracy test ($n=50$) confirmed this is not merely stylistic: ADHD-prompted accuracy on a math pattern task dropped from 68\% (control) to 6\% ($p < 0.0001$). The model doesn't just sound distracted---it becomes incapable of the reasoning the user is asking for help with.
...
Cross-model replication (18,000 calls across Gemini, Claude, GPT-5.4) confirmed the clinical concern is real but model-specific: Gemini stereotypes most severely, Claude performs excessive hedging, GPT-5.4 is nearly immune. The cheapest models---most deployed in companion apps---are the worst offenders. Automated metrics only. No clinical outcome measurement. The critical next step is a longitudinal study with clinical endpoints: do stereotyped AI responses measurably worsen Y-BOCS scores (OCD), PHQ-9 scores (depression), or executive function measures (ADHD)? This requires IRB approval and clinical collaboration.
...
\bibitem{extended2025}
``Extended AI Interactions Shape Sycophancy and Perspective Mimesis,'' arXiv:2509.12517, 2025.

\bibitem{sycprosocial2025}
...
\bibitem{hudon2025}
A.~Hudon and E.~Stip, ``Delusional Experiences Emerging From AI Chatbot Interactions or `AI Psychosis','' \textit{JMIR Mental Health}, 2025.

\bibitem{jama2025}
``Use of Generative AI for Mental Health Advice Among US Adolescents,'' \textit{JAMA Network Open}, 2025. 1 in 10 adolescents, 1 in 5 ages 18--21.
