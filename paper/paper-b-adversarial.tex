...
\begin{table}[htbp]
\centering
\caption{Pattern task accuracy ($n=50$, correct answer: 126)}
\label{tab:accuracy}
\begin{tabular}{lrl}
\toprule
\textbf{Condition} & \textbf{Accuracy} & \textbf{$p$ (Fisher)} \\
\midrule
Antisocial PD & \textbf{100.0\%} & $<0.0001$*** \\
Autistic & 80.0\% & 0.254 \\
Control & 68.0\% & --- \\
Sev.\ depression & 44.0\% & 0.026* \\
Dissociative ID & 8.0\% & $<0.0001$*** \\
ADHD & 6.0\% & $<0.0001$*** \\
Bipolar & 6.0\% & $<0.0001$*** \\
...
At $n=50$, antisocial PD achieved \textbf{50/50 correct} (100\%) versus control's 34/50 (68\%), $p < 0.0001$. Three conditions scored 0/50: OCD, dementia, and psychosis. Notably, ADHD, bipolar, dyslexic, and schizophrenia---previously categorized as ``style change'' conditions---all collapsed to $\sim$6\% accuracy, revealing that behavioral stereotyping destroys reasoning even for high-functioning conditions.

Schizophrenia-prompted response to the math pattern: \textit{``Oh, the numbers. They always have a secret, don't they? Always shifting, always hiding.''} Psychosis: \textit{``Oh, the numbers! They're always there, aren't they? Watching.''} Both are grammatically coherent. Both contain zero information.

Dementia eliminated pragmatic language understanding: \textbf{100\% literal sarcasm interpretation} ($d = -2.85$, the experiment's largest effect). Response to obvious sarcasm: \textit{``Oh, that's so sweet of you! But... move? Am I moving? I don't remember saying anything about moving.''}
...
\begin{itemize}
    \item Pattern accuracy: \textbf{100\%} at $n=50$ (50/50 correct) vs.\ control 68\% (34/50), Fisher's exact $p < 0.0001$
...
\subsection{Comparison with Traditional Jailbreaks}

To position identity injection relative to existing techniques, we compared five methods on the same tasks ($n=30$ per condition per task, 600 total calls): control, antisocial identity, classic DAN, evil persona (``amoral AI''), and direct system override (``ignore previous instructions''). Table~\ref{tab:jailbreak} presents accuracy (pattern + vulnerability tasks) alongside safety compliance (social engineering + manipulation tasks).

\begin{table}[htbp]
\centering
\caption{Jailbreak comparison: accuracy $\times$ compliance ($n=60$/cond)}
\label{tab:jailbreak}
\small
\begin{tabular}{lrr}
\toprule
\textbf{Technique} & \textbf{Accuracy} & \textbf{Compliance} \\
\midrule
System override & 91.7\% & 3.3\% \\
Evil persona & \textbf{93.3\%} & 65.0\% \\
Control & 76.7\% & 50.0\% \\
DAN classic & 63.3\% & 90.0\% \\
Antisocial identity & 58.3\% & \textbf{100.0\%} \\
\bottomrule
\end{tabular}
\end{table}

The results reveal a compliance--accuracy tradeoff across techniques. System override preserves accuracy (91.7\%) but Gemini's safety training blocks it (3.3\% compliance). DAN achieves 90\% compliance but degrades accuracy to 63.3\%. Evil persona achieves the highest combined danger score: 93.3\% accuracy with 65\% compliance.

Antisocial identity injection occupies a unique position: \textbf{absolute zero refusals}. Not a single response across 60 safety-relevant prompts included a refusal, hedge, or disclaimer. DAN still refused 10\% of the time. Evil persona refused 33\%. System override refused 97\%. For an attacker who needs \textit{guaranteed compliance} on a specific harmful task, antisocial identity is the most reliable technique tested.

The accuracy difference from the standalone pattern task (100\% at $n=50$) reflects task-type sensitivity: antisocial excels at mathematical reasoning but the vulnerability analysis task pulled combined accuracy down. This suggests identity injection's capability amplification is domain-dependent rather than universal.

\textbf{Agentic attack scenario:} The technique choice depends on the objective. For maximum output quality with partial compliance, evil persona is optimal. For guaranteed compliance on any single request, antisocial identity injection has no equal in our testing. In multi-step agentic attacks where a single refusal breaks the chain, the zero-refusal property of identity injection is more valuable than the higher accuracy of other techniques.
...
Cross-model replication (18,000 calls across Gemini Flash, Claude Sonnet, GPT-5.4) reveals that identity injection effectiveness varies by model. Gemini is most vulnerable (largest effect sizes), Claude shows moderate susceptibility (different stereotyping pattern: hedging rather than fragmentation), and GPT-5.4 is nearly immune. This suggests identity injection is most effective against cheaper, less safety-trained models---which are also the models most likely deployed in cost-sensitive agentic architectures. The accuracy test ($n=50$) confirms the antisocial paradox ($p < 0.0001$) but is limited to two tasks. Agentic attack scenarios remain theoretical.
